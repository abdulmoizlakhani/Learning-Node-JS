...
// stored all functinalities of createServer method in server vaiable.
var server = http.createServer();

// server provides 'on' method to handle requests more explicitly.
// on method takes two arguments, fist one is 'request' as string and second has callback.
...
// the server sends the response only when the request is made.

server.on('request', function(req, res){
    res.writeHead(200, {'Content-Type': 'text/plain'});
    res.end('Hello Second Test Node Server\n')
});

// server provides listen method to tell the node js about the port on which the server will run.
// I have choosed port 3000. It depend on your choice but first confirm that the port is free.
// Whenever a request happens, the function (req, res) callback is fired and “Hello Second Test Node Server” is written out as the response.
server.listen(3000);

console.log('Server running at http://localhost:3000/');

// cd to working directory, e.g. 1_hello_world_http_server
// run command: "node test2_server.js" in the terminal to start server.
